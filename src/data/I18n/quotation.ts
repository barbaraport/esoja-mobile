export default {
  pt: {
    quotation: {
      seedPageTitle: 'Cotações das sementes',
      seedPageDescription: 'Valores baseados no preço da semente convencional do estado do Mato Grosso do Sul',
      bagPageTitle: 'Cotações das sacas',
      pageDescription:
        'Selecione uma cidade e fique por dentro das últimas atualizações do custo da soja',
      bagPageDescription: 'Valores baseados no preço da soja disponível do estado do Mato Grosso do Sul',
      selectTitle: 'Selecione a cidade',
      date: 'Data',
      bagPrice: 'Preço sc 60kg',
      seedPrice: 'Preço sementes',
      variation: 'Variação',
      chartTitle: 'Variação do preço da soja R$ (sc 60kg)',
      seedTitleFooter: 'Sementes',
      bagTitleFooter: 'Saca'
    }
  },
  en: {
    quotation: {
      seedPageTitle: 'Seeds quotations',
      seedPageDescription: 'Values based on the price of conventional seed in the state of Mato Grosso do Sul',
      bagPageTitle: 'Bags quotations',
      bagPageDescription: 'Values based on the price of available soybeans in the state of Mato Grosso do Sul',
      pageDescription:
        'Select a city and stay up to date with the latest soy cost updates',
      selectTitle: 'Select city',
      date: 'Date',
      bagPrice: 'Price Bag 60kg',
      seedPrice: 'Price seeds',
      variation: 'Variation',
      chartTitle: 'Soybean (bag 60kg) price variation',
      seedTitleFooter: 'Seeds',
      bagTitleFooter: 'Bag'
    }
  }
};
